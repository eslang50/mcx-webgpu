...
(async function main() {
  ...
  let baseShaderPath = `ray_tracing/shaders`;
  ...
  let rayGenShaderModule = device.createShaderModule({
    code: fs.readFileSync(`${baseShaderPath}/ray_generation.rgen`, "utf-8")
  });
  let rayCHitShaderModule = device.createShaderModule({
    code: fs.readFileSync(`${baseShaderPath}/ray_closest-hit.rchit`, "utf-8")
  });
  let rayMissShaderModule = device.createShaderModule({
    code: fs.readFileSync(`${baseShaderPath}/ray_miss.rmiss`, "utf-8")
  });
